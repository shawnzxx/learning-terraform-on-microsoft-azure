...
resource "azurerm_virtual_machine_scale_set" "web_server" {
  ...
  os_profile {
    #   computername only allow for 15 chars
  ...
  os_profile_windows_config {
    ...
    # VM Extensions enable post-deployment configuration of VM, such as installing and configuring software. 
  ...
  network_profile {
    ...
    ip_configuration {
      ...
      primary                                = true
      # we have 2 subnet here we only use subnet 0
  ...
  # for VM post configuration, install software configure web server etc.
  extension {
